...
async function migrate(): Promise<void> {
  ...
  await sql`CREATE TABLE IF NOT EXISTS notifications (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE, title varchar(160) NOT NULL, message text NOT NULL, is_read boolean NOT NULL DEFAULT false, created_at timestamptz NOT NULL DEFAULT now(), hidden_from_bell boolean NOT NULL DEFAULT false)`;
  ...
  await sql`ALTER TABLE leads ADD COLUMN IF NOT EXISTS customer_id uuid REFERENCES clients(id)`;
  await sql`ALTER TABLE leads ADD COLUMN IF NOT EXISTS converted_at timestamptz`;
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS leads_customer_id_unique ON leads(customer_id) WHERE customer_id IS NOT NULL`;
  ...
  await sql`ALTER TABLE projects ADD COLUMN IF NOT EXISTS progress_percentage integer NOT NULL DEFAULT 0 CHECK (progress_percentage BETWEEN 0 AND 100)`;
  ...
  await sql`ALTER TABLE project_updates ADD COLUMN IF NOT EXISTS old_status varchar(40)`;
